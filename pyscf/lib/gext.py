...
class Extrapolator:
    ...
    def __init__(self, nelectrons, max_npoints=50, regularization=1e-4,
            fitting="descriptor_fitting", order=1):
        ...
        self.coefficients = []
        self.overlaps = []
        if fitting == "descriptor_fitting":
            if order > 1:
                raise ValueError("descriptor_fitting is only first order.")
            self.fit = DescriptorFitting(regularization=regularization)
        elif fitting == "regression":
            self.fit = PolyomialRegression(regularization=regularization,
                order=order)
        else:
...
class PolyomialRegression:

    """Polynomial regression fitting."""

    def __init__(self, regularization=1e-4, order=1):
        if order not in [1, 2]:
    ...
    def train(self, vectors, gammas):
        ...
        gammas = np.array([gamma.flatten() for gamma in gammas[:-1]]).T
